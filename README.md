...
```bash
# 抓取最新訊息（自上次抓取時間至今，最多回溯一天）
node index.js fetch

# 抓取歷史區間：N 天前 ~ N-1 天前，檔名使用 N-1 天前日期
node index.js fetch <N>
```

**範例：**
```bash
node index.js fetch 3   # 抓取 3天前00:00 ~ 2天前00:00 的訊息，檔名為 2天前
node index.js fetch 7   # 抓取 7天前 ~ 6天前
```
...
| 情況 | since 起始時間 |
|------|--------------|
| 正常模式（無參數） | 上次抓取時間；若從未抓取或超過兩天，則從一天前開始 |
| 歷史模式（`fetch N`） | N 天前；結束時間 = N-1 天前（client-side 過濾） |
...
```bash
# 補抓昨天（2天前~1天前不行，最少需傳2）
node index.js fetch 2   # 2天前~昨天

# 補抓更早的歷史
node index.js fetch 3   # 3天前~2天前
node index.js fetch 7   # 7天前~6天前
```
...
### `src/fetcher.js`
核心抓取邏輯。支援正常模式（依 last fetch state）和歷史模式（自訂時間範圍）。歷史模式在 client-side 過濾上界時間。
