...
description: 針對程式設計師必讀文章生成雙語（英中對照）HTML 導讀。當用戶說「翻譯文章」、「生成雙語導讀」、「translate articles」、「teams-ai-translate」時觸發。從 ./dailyReport/ 最新的 HTML 讀取必讀文章清單，抓取英文文章內容並生成雙語 HTML 到 ./dailyReport/{日期}/ 目錄。
---
...
### 選項 1：從 dailyReport/ 最新 HTML 自動讀取（推薦）

使用 Bash 列出 `./dailyReport/` 目錄下所有 `.html` 檔案，取檔名排序最後一個（即最新日報）。
...
- 文章 URL
- 文章標題（可選，用於檔案命名）
- 輸出檔案路徑（可選，預設為 `./dailyReport/{date}/article_N.html`）
...
**檔案命名規則：**
- 從 dailyReport/ 最新 HTML 讀取：`./dailyReport/{date}/article_{rank}.html`（如 `./dailyReport/2026-03-19/article_1.html`）
- `{date}` 取自最新日報檔案名稱（如 `daily_report_2026-03-19.html` → `2026-03-19`）
- 輸出前先執行 `mkdir -p ./dailyReport/{date}/` 確保目錄存在
- 用戶直接提供：`./dailyReport/{date}/article_{自動編號}.html` 或用戶指定路徑

**檔案內容：** 完整的 HTML 文件（見下方模板）

### 5. 更新日報 HTML（可選）

如果文章來自 dailyReport/ 最新日報的必讀清單，在對應的 `<div class="featured-item">` 中加入雙語連結：
...
### 全部翻譯（預設）
從 `./dailyReport/` 最新日報 HTML 讀取所有必讀文章，逐一處理：
1. 檢查是否已有雙語導讀（避免重複）
2. 抓取文章內容
3. 判斷語言
4. 若為英文，生成雙語 HTML 到 `./dailyReport/{date}/`
5. 更新最新日報 HTML 加入連結
6. 輸出翻譯狀態報告
...
- **檔案管理**：確保 `./dailyReport/{date}/` 目錄存在（用 `mkdir -p`），不覆蓋已有檔案（除非用戶明確要求）
...
📝 已生成 2 篇雙語導讀：
  • dailyReport/2026-03-19/article_3.html - mcp2cli: 節省 90% Token 的 CLI 工具
  • dailyReport/2026-03-19/article_4.html - GitHub Copilot Instructions Files 完整指南
...
💡 建議：在瀏覽器中開啟 dailyReport/daily_report_2026-03-19.html，點擊「中文導讀 →」查看翻譯結果。
```
...
原因：無法在 ./dailyReport/ 找到任何 HTML 檔案
