...
description: 針對程式設計師必讀文章生成雙語（英中對照）HTML 導讀。當用戶說「翻譯文章」、「生成雙語導讀」、「translate articles」、「teams-ai-translate」時觸發。可選擇性指定日期（如「翻譯 2026-03-18 的文章」），從 ./docs/ 對應日期（或最新）的 HTML 讀取必讀文章清單，生成雙語 HTML 到 ./docs/{日期}/ 目錄。
---
...
**若有指定日期**：讀取 `./docs/daily_report_{date}.html`，輸出到 `./docs/{date}/`。

**若未指定日期**：使用 `./docs/` 下最新的日報 HTML（預設行為）。

日報選擇邏輯：
```bash
# 有指定日期
ls ./docs/daily_report_2026-03-18.html

# 未指定日期 → 取最新
ls ./docs/daily_report_*.html | sort | tail -1
```
...
### 選項 1：從 docs/ 指定（或最新）HTML 自動讀取（推薦）

依上方日期參數邏輯決定目標日報，使用 Bash 找到對應的 `daily_report_*.html`。
...
**檔案命名規則：**
- `./docs/{date}/article_{rank}.html`（如 `./docs/2026-03-19/article_1.html`）
- `{date}` 優先使用用戶指定日期；若未指定，取自所讀日報的檔案名稱（如 `daily_report_2026-03-19.html` → `2026-03-19`）
...
### 全部翻譯（預設）
從 `./docs/` 指定日期（或最新）日報 HTML 讀取所有必讀文章，逐一處理：
1. 檢查是否已有雙語導讀（避免重複）
2. 抓取文章內容
3. 判斷語言
