---
name: teams-ai-daily
description: 生成 Teams AI 頻道每日新聞日報。當用戶想看 AI 新聞概覽、每日 AI 日報、Teams AI 新聞摘要，或直接說「產生日報」、「今天有什麼AI新聞」、「teams-ai-daily」時觸發。可選擇性指定日期（如「產生 2026-03-18 的日報」），讀取 archive/AI news/ 下對應日期的 JSON，過濾整理後輸出精美 HTML 報告到 ./docs/daily_report_YYYY-MM-DD.html。
---
...
**若有指定日期**：在 `archive/AI news/` 中找檔名包含該日期（`YYYY-MM-DD`）的 JSON；若無完全符合，取檔名日期最接近且不超過指定日期的那一個。

**若未指定日期**：使用最新的 JSON 檔案（預設行為）。

判斷方式：
```bash
node -e "
const fs = require('fs');
const targetDate = process.argv[1]; // 'YYYY-MM-DD' 或 null
const files = fs.readdirSync('./archive/AI news')
  .filter(f => f.endsWith('.json'))
  .sort();
let chosen;
if (targetDate) {
  // 找包含指定日期的檔案，或最接近且不超過的
  const matched = files.filter(f => f.includes(targetDate));
  chosen = matched.length > 0 ? matched[matched.length - 1]
    : files.filter(f => f < 'messages_' + targetDate + 'T99').pop();
} else {
...
將結果寫入 `./docs/daily_report_YYYY-MM-DD.html`，日期取自資料中最新訊息的台灣時間（UTC+8，格式 `YYYY-MM-DD`）。若 `docs/` 目錄不存在，先用 `mkdir -p` 建立。
...
- **輸出路徑**：主日報寫到 `./docs/daily_report_YYYY-MM-DD.html`；日期優先使用用戶指定的日期，若未指定則取資料中最新訊息的台灣時間（UTC+8）。完成後告知用戶路徑並提示用瀏覽器開啟
